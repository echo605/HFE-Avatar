...
from animatableGaussian.enhanced_losses import (
    MultiScaleSSIMLoss,
    FrequencyDomainLoss,
    CombinedImageLoss
)
...
class NeRFModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        self.combined_loss = CombinedImageLoss().cuda()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_enhanced, enhanced_losses_dict = self.combined_loss(image, gt_image)

        Ll1 = l1_loss(image, gt_image)
        lpips_loss_val = self.evaluator.psnr(image.unsqueeze(0), gt_image.unsqueeze(0)).mean()

        loss = (1.0 - self.lambda_dssim) * Ll1 + self.lambda_dssim * loss_enhanced - 0.0001*lpips_loss_val
    ...
    def validation_step(self, batch, batch_idx):
        ...
        rgb_gt = batch["gt"]
        loss_enhanced, enhanced_losses_dict = self.combined_loss(rgb, rgb_gt)

        Ll1 = l1_loss(
            rgb, rgb_gt)
        lpips_loss_val = self.evaluator.psnr(rgb.unsqueeze(0), rgb_gt.unsqueeze(0)).mean()

        val_loss = (1.0 - self.lambda_dssim) * Ll1 + self.lambda_dssim * loss_enhanced - 0.0001*lpips_loss_val
